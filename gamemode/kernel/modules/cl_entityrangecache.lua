local lib = mono.util.G("EntityDraw", {})
...
hook.Add("PostDrawTranslucentRenderables", "CachedPostDrawTrans", function()
...
hook.Add("PostPlayerDraw", "CachedPostPlayerDraw", function()
	-- Call the DrawEntityUI hook on all valid ents in the cache.
	for index = 1, #lib.EntitiesInRange do
		local tEntData = lib.EntitiesInRange[index]
		if tEntData and IsValid(tEntData.ent) then
...
hook.Add("PostDrawOpaqueRenderables", "CachedPostDrawOpaqueRenderables", function()
	-- Call the DrawEntityUI hook on all valid ents in the cache.
	for index = 1, #lib.EntitiesInRange do
		local tEntData = lib.EntitiesInRange[index]
		if tEntData and IsValid( tEntData.ent ) then
