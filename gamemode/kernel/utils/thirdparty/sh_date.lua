...
local function error_dup(q) error("duplicate value: " .. (q or "") .. sw:aimchr()) end
local function error_syn(q) error("syntax error: " .. (q or "") .. sw:aimchr()) end
local function error_inv(q) error("invalid date: " .. (q or "") .. sw:aimchr()) end
...
local tvspec = {['%a']=function(self) return sl_weekdays[weekday(self.daynum) + 7] end,['%A']=function(self) return sl_weekdays[weekday(self.daynum)] end,['%b']=function(self) return sl_months[self:getmonth() - 1 + 12] end,['%B']=function(self) return sl_months[self:getmonth() - 1] end,['%C']=function(self) return fmt("%.2d", fix(self:getyear()/100)) end,['%d']=function(self) return fmt("%.2d", self:getday())  end,['%g']=function(self) return fmt("%.2d", mod(self:getisoyear() ,100)) end,['%G']=function(self) return fmt("%.4d", self:getisoyear()) end,['%h']=function(self) return self:fmt0("%b") end,['%H']=function(self) return fmt("%.2d", self:gethours())end,['%I']=function(self) return fmt("%.2d", self:getclockhour()) end,['%j']=function(self) return fmt("%.3d", self:getyearday())end,['%m']=function(self) return fmt("%.2d", self:getmonth())end,['%M']=function(self) return fmt("%.2d", self:getminutes())end,['%p']=function(self) return sl_meridian[self:gethours() > 11 and 1 or -1] end,['%S']=function(self) return fmt("%.2d", self:getseconds())  end,['%u']=function(self) return self:getisoweekday() end,['%U']=function(self) return fmt("%.2d", self:getweeknumber()) end,['%V']=function(self) return fmt("%.2d", self:getisoweeknumber()) end,['%w']=function(self) return self:getweekday() - 1 end,['%W']=function(self) return fmt("%.2d", self:getweeknumber(2)) end,['%y']=function(self) return fmt("%.2d", mod(self:getyear() ,100)) end,['%Y']=function(self) return fmt("%.4d", self:getyear()) end,['%z']=function(self) local b = -self:getbias(); local x = abs(b); return fmt("%s%.4d", b < 0 and "-" or "+", fix(x/60)*100 + floor(mod(x,60))) end,['%Z']=function(self) return self:gettzname() end,['%\b']=function(self) local x = self:getyear() return fmt("%.4d%s", x>0 and x or (-x+1), x>0 and "" or " BCE") end,['%\f']=function(self) local x = self:getfracsec() return fmt("%s%.9f",x >= 10 and "" or "0", x) end,['%%']=function(self) return "%" end,['%r']=function(self) return self:fmt0("%I:%M:%S %p") end,['%R']=function(self) return self:fmt0("%I:%M")  end,['%T']=function(self) return self:fmt0("%H:%M:%S") end,['%D']=function(self) return self:fmt0("%m/%d/%y") end,['%F']=function(self) return self:fmt0("%Y-%m-%d") end,['%c']=function(self) return self:fmt0("%x %X") end,['%x']=function(self) return self:fmt0("%a %b %d %\b") end,['%X']=function(self) return self:fmt0("%H:%M:%\f") end,['${iso}'] = function(self) return self:fmt0("%Y-%m-%dT%T") end,['${http}'] = function(self) return self:fmt0("%a, %d %b %Y %T GMT") end,['${ctime}'] = function(self) return self:fmt0("%a %b %d %T GMT %Y") end,['${rfc850}'] = function(self) return self:fmt0("%A, %d-%b-%y %T GMT") end,['${rfc1123}'] = function(self) return self:fmt0("%a, %d %b %Y %T GMT") end,['${asctime}'] = function(self) return self:fmt0("%a %b %d %T %Y") end,}
